...
CAPTURE_DOWNLOAD_DIR=./gfxr_capture
...
trigger_capture() {
  ...
  while true; do
    ...
    mkdir -p ${CAPTURE_DOWNLOAD_DIR}
    adb pull ${COMPOSITOR_CAPTURE_DIR} ${CAPTURE_DOWNLOAD_DIR}
...
# XrPassthroughController:
#   enabled: true
#   passthroughRunning: 1
is_passthrough() {
  output=$(adb shell dumpsys cpm | grep -A 2 "XrPassthroughController:")
  if echo "$output" | grep -q "passthroughRunning:"; then
    val=$(echo "$output" | awk '/passthroughRunning:/ {print $2}')
    [ "$val" != "0" ]
  elif echo "$output" | grep -q "running:"; then
    val=$(echo "$output" | awk '/running:/ {print $2}')
    [ "$val" == "true" ]
  else
    false
  fi
